...
fn test_wots_pkgen() {
	...
	for i, item in tests {
		...
		actual_pk := wots_pkgen(mut c, skseed, pkseed, mut adrs)!
...
fn test_wots_sign_verify() ! {
	...
	for i, item in tests {
		...
		actual_sig := wots_sign(mut c, message, skseed, pkseed, mut adrs)!

		// gets flatten-ed actual_sig
		flatten_sig := arrays.flatten[u8](actual_sig)
		assert flatten_sig == expected_sig

		public_key := wots_pkgen(mut c, skseed, pkseed, mut adrs)!
		// wots_pkfromsig(c &Context, sig []u8, m []u8, pkseed []u8, mut adr Address)
		recovered_pk := wots_pkfromsig(mut c, actual_sig, message, pkseed, mut adrs)!
