...
fn test_xmms_node() ! {
	...
	for item in tests {
		...
		node := xmss_node(mut c, skseed, 0, 3, pkseed, mut addr)!
...
fn test_xmms_sign() ! {
	...
	for item in tests {
		...
		xmss_sig := xmss_sign(mut c, m, skseed, item.idx, pkseed, mut addr)!
		...
		pk_from_sig := xmms_pkfromsig(mut c, item.idx, xmss_sig, m, pkseed, mut addr)!
